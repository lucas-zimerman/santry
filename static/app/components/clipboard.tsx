...
function Clipboard({
  ...
  const elementRef = useRef<ReturnType<typeof findDOMNode>>();
  ...
  const attachClickHandler = useCallback(() => {
    ...
    return () => elementRef.current?.removeEventListener('click', handleClick);
  }, [handleClick]);

  // Handle click handler cleanup / reattachment
  useEffect(() => attachClickHandler(), [attachClickHandler]);

  const handleMount = useCallback(
    (ref: HTMLElement) => {
      // eslint-disable-next-line react/no-find-dom-node
      elementRef.current = findDOMNode(ref);
      attachClickHandler();
    },
    [attachClickHandler]
  );
